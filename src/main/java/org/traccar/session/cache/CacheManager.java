...
import com.tms.utils.primitive.hopscotch.SortedLongSet;
...
public class CacheManager implements BroadcastInterface {
    ...
    private final PrimitiveLongIntMap deviceReferences = Primitive.longIntMap();
    private final PrimitiveLongObjectMap<Map<Class<? extends BaseModel>, SortedLongSet>>
            deviceLinks = Primitive.longObjectMap();
    private final PrimitiveLongObjectMap<Position> devicePositions = Primitive.longObjectMap();

    private Server server;
    private final PrimitiveLongObjectMap<List<User>> notificationUsers = Primitive.longObjectMap();
    ...
    public <T extends BaseModel> List<T> getDeviceObjects(long deviceId, Class<T> clazz) {
        try {
            ...
            if (links != null) {
                ...
                var linkedIds = links.get(clazz);
                if (linkedIds != null) {
                    linkedIds.forEach(id -> {
                        var cacheValue = deviceCache.get(new CacheKey(clazz, id));
                        if (cacheValue != null)
                            result.add(cacheValue.<T>getValue());
                    });
                }
                return result;
                //return links.getOrDefault(clazz, )
                /*
    ...
    public List<User> getNotificationUsers(long notificationId, long deviceId) {
        try {
            lock.readLock().lock();
            PrimitiveLongSet linkedDevices = Primitive.longSet();

            var users = deviceLinks.get(deviceId).get(User.class);
            var notifyUsers = notificationUsers.get(notificationId);
            if (notifyUsers != null) {
                return notifyUsers.stream()
    ...
    public void addDevice(long deviceId) throws StorageException {
        try {
            lock.writeLock().lock();
            int references = deviceReferences.get(deviceId);
            if (references > 0) {
                references += 1;
            } else {
    ...
    private void unsafeAddDevice(long deviceId) throws StorageException {
        Map<Class<? extends BaseModel>, SortedLongSet> links = new HashMap<>();
        ...
        if (device != null) {
            ...
            if (device.getCalendarId() > 0) {
                ...
                links.computeIfAbsent(Calendar.class, k -> Primitive.sortedLongSet())
                        .add(calendar.getId());
            ...
            while (groupDepth < GROUP_DEPTH_LIMIT && groupId > 0) {
                ...
                links.computeIfAbsent(Group.class, k -> Primitive.sortedLongSet()).add(group.getId());
            ...
            for (Class<? extends BaseModel> clazz : CLASSES) {
                ...
                SortedLongSet set = Primitive.sortedLongSet(objects.size());
                objects.stream().map(BaseModel::getId).forEach(set::add);
                for (var object : objects) {
                    ...
                    if (object instanceof Schedulable) {
                        ...
                        if (scheduled.getCalendarId() > 0) {
                            ...
                            links.computeIfAbsent(Calendar.class, k -> Primitive.sortedLongSet()).add(calendar.getId());
                            addObject(deviceId, calendar);
                        }
                    }
                }
            ...
            SortedLongSet set = Primitive.sortedLongSet(users.size());
            users.stream().map(BaseModel::getId).forEach(set::add);
            links.put(User.class, set);
            for (var user : users) {
                ...
                for (var notification : notifications) {
                    links.computeIfAbsent(Notification.class, k -> Primitive.sortedLongSet()).add(notification.getId());
    ...
    private void unsafeInvalidate(CacheKey[] keys) throws StorageException {
        ...
        for (var key : keys) {
            if (key.classIs(Server.class)) {
                ...
                deviceCache.computeIfPresent(key, (k, value) -> {
                    linkedDevices.addAll(value.getReferences().iterator());
                    return value;
                });
            }
        }
        var iter = linkedDevices.iterator();
        for (;iter.hasNext();) {
            var deviceId =  iter.next();
            unsafeRemoveDevice(deviceId);
            unsafeAddDevice(deviceId);
        }
        if (invalidateServer) {
    ...
    public Device getDevice(long deviceId){
        return getObject(Device.class, deviceId);
    }

    public <E extends Exception>
        void visitDevicePosition(PrimitiveLongObjectVisitor<Position, E> visitor) throws E {
        try {
            lock.writeLock().lock();
            devicePositions.visitEntries(visitor);
