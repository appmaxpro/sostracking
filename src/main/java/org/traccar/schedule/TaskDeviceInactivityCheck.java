...
public class TaskDeviceInactivityCheck implements ScheduleTask {
    ...
    public void run() {
        ...
        try {
            ...
            long minTime = currentTime - checkPeriod;
            long maxTime = currentTime - TimeUnit.MINUTES.toMillis(1);

            Request.builder()
                    .and(Condition.notNull("lastUpdate"))
                    .and(Condition.gte("lastUpdate", new java.sql.Timestamp(minTime)))
                    .and(Condition.lte("lastUpdate", new java.sql.Timestamp(maxTime)));
