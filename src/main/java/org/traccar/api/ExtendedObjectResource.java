    ...
    public Collection<T> get(
        ...
        if (all) {
            ...
            if (userId == 0) {
                ...
                builder.and(new Condition.Permission(User.class, userId, baseClass).excludeGroups());
            }
        }

        if (groupId > 0) {
            permissionsService.checkPermission(Group.class, getUserId(), groupId);
            builder.and(new Condition.Permission(Group.class, groupId, baseClass).excludeGroups());
        }
        if (deviceId > 0) {
            permissionsService.checkPermission(Device.class, getUserId(), deviceId);
            builder.and(new Condition.Permission(Device.class, deviceId, baseClass).excludeGroups());
        }
