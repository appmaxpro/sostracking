...
public class DatabaseStorage extends Storage {
    ...
    public <T> List<T> getObjects(Class<T> clazz, Request request) throws StorageException {
        ...
        query.append(formatLimit(request));
        try {
    ...
    private String formatLimit(Request request) {
        if ((request.getOrder() == null || request.getOrder().getLimit() == 0 )
        && (request.getOffset() != null || request.getLimit() != null)) {
            ...
            else if (request.getLimit() != null && request.getLimit() > 0){
                if (databaseType.equals("Microsoft SQL Server")) {
                    result.append(" FETCH FIRST ")
                            .append(request.getLimit())
                            .append(" ROWS ONLY");
