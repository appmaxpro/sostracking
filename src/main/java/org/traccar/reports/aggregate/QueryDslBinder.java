...
public class QueryDslBinder {
	...
	private static class ClassConfiguration<T> {
		...
		public Constructor<T> getNoArgumentConstructor() {
			return noArgumentConstructor;
		}

		public void setNoArgumentConstructor(Constructor<T> noArgumentConstructor) {
			this.noArgumentConstructor = noArgumentConstructor;
		}

		public GroupByBinder getGroupByBinder() {
			return groupByBinder;
		}

		public List<T> getOrderedRows() {
			return orderedRows;
		}
	...
	private class InstancedRow<T> {
		...
		public InstancedRow(RowKey rowKey, T instance, boolean cached) {
			this.rowKey = rowKey;
	...
	public static <T> List<T> to(List<Tuple> tupleList, Class<T> dtoClass, GroupByBinder groupByBinder) {
		...
		try {
			...
			ClassConfiguration<T> classConfiguration =
					instance.getClassConfiguration(dtoClass, groupByBinder);
	...
	private <T> ClassConfiguration<T> getClassConfiguration(Class<T> dtoClass,
															GroupByBinder groupBy)
