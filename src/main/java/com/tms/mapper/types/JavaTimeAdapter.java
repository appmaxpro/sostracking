...
public class JavaTimeAdapter implements TypeAdapter<Object> {
  ...
  public Object adapt(Object value, Class<?> actualType, Type genericType, Property property) {
    ...
    if (ZonedDateTime.class.isAssignableFrom(actualType))
      return dateTimeAdapter.adapt(value, actualType, genericType, property);

    if (LocalDate.class.isAssignableFrom(actualType))
      return localDateAdapter.adapt(value, actualType, genericType, property);

    if (LocalTime.class.isAssignableFrom(actualType))
      return localTimeAdapter.adapt(value, actualType, genericType, property);

    if (LocalDateTime.class.isAssignableFrom(actualType))
      return localDateTimeAdapter.adapt(value, actualType, genericType, property);
